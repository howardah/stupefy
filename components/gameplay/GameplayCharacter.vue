...
const healthPips = computed(() => Array.from({ length: props.character.health }, (_, index) => index));
</script>
...
<template>
  <div class="flex flex-col items-center gap-2">
    <button
      ...
      :class="[
        clickable ? 'cursor-pointer hover:-translate-y-1 hover:shadow-[0_18px_36px_rgba(61,39,16,0.18)]' : '',
        selected ? 'ring-2 ring-[rgba(180,83,9,0.75)] ring-offset-2 ring-offset-[rgba(245,239,226,0.9)]' : '',
      ]"
      :style="{
      ...
      @click="emit('click')"
    >
      <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[rgba(20,12,7,0.92)] via-[rgba(20,12,7,0.6)] to-transparent px-3 py-3 text-left text-white">
        <div class="text-sm font-semibold">{{ character.shortName }}</div>
        <div class="text-[10px] uppercase tracking-[0.18em] text-white/75">{{ character.house || "No house" }}</div>
      </div>
    </button>

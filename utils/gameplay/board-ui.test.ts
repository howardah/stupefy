...
describe("board UI helpers", () => {
  ...
  test("checkPlayerDistance honors wand range and opposing distance effects", () => {
    ...
    players[0]!.tableau = [{ id: 1, name: "larch_wand", fileName: "larch_wand", house: "G", power: { range: 4 } }];
    players[1]!.tableau = [{ id: 2, name: "broomstick", fileName: "broomstick", house: "S", power: { distance: 1 } }];
  ...
  test("target clickability helpers reflect the available target tokens", () => {
    ...
    expect(isTableauTargetClickable(opponent, 11, ["tableau"], state.players, tableauCard)).toBe(true);
